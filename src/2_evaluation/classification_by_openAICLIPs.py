...
def extract_features_only_text(model, tokeniser, text_list, device="cuda"): #-> Dict[str, torch.Tensor]:
    ...
    with torch.no_grad():
        ...
        #    outputs = model(**inputs) 
        text_tokens = tokeniser(text_list).to(device) 
...
def process_sample(
    ...
    input_captions = tokeniser([v["Meaning"] for v in variants]).to(device) 
    input_images = [processor(Image.open(image_root / ambiguity_type / v["Image"]).convert("RGB")) for v in variants] 
...
def evaluate_model(args: argparse.Namespace) -> None: 
    ...
    for ambiguity_type in TYPE_NAMES: 
        ...
        output_path = output_dir / f"results_{model_card}_{pretrained_data}_{ambiguity_type}.pkl" 
