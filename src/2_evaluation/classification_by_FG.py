...
def extract_features(model, text_list, image_list=None, device="cuda") -> Dict[str, torch.Tensor]:
    ...
    print("image_size:", model.vision_model.config.image_size)
    print("patch_size:", model.vision_model.config.patch_size)
    #print("num_positions:", model.vision_model.config.num_positions)
    
    print("pixel_values shape:", image_list.shape)


    
    with torch.no_grad(), torch.cuda.amp.autocast():
        image_features = model.get_image_features(image_list)#.float()
        text_features = model.get_text_features(text_list)#.float() 
...
def process_sample(
    ...
    input_images = processor.preprocess(
        images = [Image.open(image_root / ambiguity_type / v["Image"]).convert("RGB").resize((336, 336)) for v in variants],
        return_tensors="pt"
...
def evaluate_model(args: argparse.Namespace) -> None: 
    ...
    model_card = "qihoo360/fg-clip-large"
    ...
    processor = AutoImageProcessor.from_pretrained("openai/clip-vit-large-patch14", use_fast=False) 
